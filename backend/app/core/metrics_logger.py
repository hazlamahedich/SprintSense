...
class MetricsLogger:
    ...
    def log_recommendation_generated(
        ...
        RESPONSE_TIME_HISTOGRAM.labels(team_id=team_id, endpoint="generate").observe(
            processing_time
        )
    ...
    def log_recommendation_accepted(
        self, team_id: str, recommendation_id: str, acceptance_rate: float
    ) -> None:
    ...
    def log_metrics_request(
        ...
        RESPONSE_TIME_HISTOGRAM.labels(team_id=team_id, endpoint="metrics").observe(
            processing_time
        )
