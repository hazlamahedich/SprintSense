...
describe('RegisterPage', () => {
  ...
  it('shows validation error for mismatched passwords', async () => {
    ...
    await waitFor(async () => {
      expect(
        await screen.findByLabelText('Confirm Password', { exact: true })
  ...
  it('handles 409 conflict error (email already exists)', async () => {
    ...
    await waitFor(async () => {
      expect(await screen.findByLabelText(/email address/i)).toHaveAttribute(
        'aria-invalid',
        'true'
      )
    })
  ...
  it('shows validation error for invalid full name with numbers', async () => {
    ...
    await waitFor(() => {
      expect(fullNameField).toHaveAttribute('aria-invalid', 'true')
      expect(
        screen.getByText(/full name can only contain letters/i)
      ).toBeInTheDocument()
    })
  })
