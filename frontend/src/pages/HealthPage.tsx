...
export const HealthPage: React.FC = () => {
  ...
  const checkDetailedHealth = async () => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Detailed health check failed'
      setError(errorMsg)
    ...
    <Container maxWidth="lg">
      <Box sx={{ py: 4 }}>
                  ...
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                      ...
                      <Chip 
                        ...
                        color={detailedHealth.status === 'OK' ? 'success' : 'error'}
                        variant="outlined"
                      />
                    </Box>
                    ...
                    {detailedHealth.database && (
                      <Typography variant="body1" sx={{ mb: 1 }}>
                        ...
                        <Chip 
                          ...
                          color={detailedHealth.database === 'connected' ? 'success' : 'error'}
                          size="small"
                          sx={{ ml: 1 }}
                        />
                      </Typography>
                    ...
                    {detailedHealth.lastChecked && (
                      <Typography variant="body2" color="text.secondary">
                        Last checked: {detailedHealth.lastChecked.toLocaleTimeString()}
                      </Typography>
                    )}
                  </Box>
                ) : !loading && (
        ...
        <Box sx={{ mt: 4, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
          <Typography variant="body2" color="text.secondary">
            This page demonstrates the connectivity between the frontend and backend services.
            The basic health check tests the API endpoint, while the detailed health check 
