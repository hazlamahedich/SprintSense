...
export class GoalService {
  ...
  static async updateGoal(
    ...
    try {
      const response = await api.put(`/teams/${teamId}/goals/${goalId}`, goalData);
      return response.data;
    } catch (error) {
  ...
  static validateGoalData(goalData: Partial<ProjectGoalCreateRequest>): {
    ...
    if (goalData.priority_weight === undefined || goalData.priority_weight === null) {
    ...
    if (goalData.success_metrics && goalData.success_metrics.trim().length > 1000) {
   ...
   */
  static getCharacterCount(text: string): { count: number; remaining: number; maxLength: number } {
   ...
   */
  static getSuccessMetricsCharacterCount(text: string): { count: number; remaining: number; maxLength: number } {
    const count = text.length;
