...
async function createTestUser() {
  try {
    ...
    const { data: authUser, error: authError } = await supabase.rpc('create_test_user', {
...
export async function ensureTestUser() {
  try {
    ...
    const { data: signInData, error: signInError } = await supabase.auth.signInWithEmailAndPassword({
